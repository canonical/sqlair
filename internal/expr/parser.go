...
func (p *Parser) parseTargets() ([]fullName, bool, error) {
	...
	if types, ok, err := p.parseTarget(); err != nil {
		return nil, false, err
	} else if ok {
		return []fullName{types}, true, nil
	}

	// Case 2: Multiple types e.g. "(&Person.name, &Person.id)".
	if types, ok, err := p.parseList((*Parser).parseTarget); err != nil {
		return nil, false, err
	} else if ok {
...
func (p *Parser) parseOutputExpression() (*outputPart, bool, error) {
	...
	if types, ok, err := p.parseTargets(); err != nil {
		return nil, false, err
	} else if ok {
		return &outputPart{
			columns: []fullName{},
...
func (p *Parser) parseInputExpression() (*inputPart, bool, error) {
	...
	if p.skipByte('$') {
		if fn, ok, err := p.parseGoFullName(); ok {
			...
			return &inputPart{typ: fn, raw: p.input[cp.pos:p.pos]}, true, nil
		} else if err != nil {
			return nil, false, err
		}
	}
