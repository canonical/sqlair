...
WHERE x = $Address.`,
		err: `cannot parse expression: line 2, column 20: invalid suffix following "Address"`,
	}, {
		query: `SELECT foo
FROM t /* multiline
comment */ WHERE x = $Address.&d`,
		err: `cannot parse expression: line 3, column 31: invalid suffix following "Address"`,
	}, {
		query: "SELECT foo FROM t WHERE x = $Address.-",
		err:   `cannot parse expression: column 38: invalid suffix following "Address"`,
	}, {
		query: "SELECT foo FROM t WHERE x = $Address",
