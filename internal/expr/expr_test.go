...
func (s *ExprSuite) TestValidComplete(c *C) {
	...
	for _, t := range tests {
		parser := expr.NewParser()
		parsedExpr, err := parser.Parse(t.sql)
		if err != nil {
			c.Fatal(err)
		}
